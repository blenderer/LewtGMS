define(['knockout', 'underscore'], function(ko, _){
	...
	self.parseRollsInString = function(rollString) {

		var patt=/\[\d+\,\d+\]/igm;

		if (rollString.match(patt)) {
			var match = patt.exec(rollString);

			var commaPattern = /(\d+)\,(\d+)/;

			var index = match.index
			var lastIndex = patt.lastIndex;
			var charLength = lastIndex - index;

			var thisMatch = rollString.substr(index + 1, charLength - 2);
			
			var op1 = thisMatch.match(commaPattern)[1];
			var op2 = thisMatch.match(commaPattern)[2];

			var dieResult = self.getRandomInt(op1*1, op2*1);

			rollString = rollString.substring(0, index) + dieResult + rollString.substring(lastIndex);
		}

		if (rollString.match(patt)) {
			rollString = self.parseRolls(rollString);
		}
