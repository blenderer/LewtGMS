define([
    ...
    function ContentCollection(name, structure, collectionArray, callbackArray) {
        ...
        var newCollectionArray = ko.observableArray();
        for (var i=0; i<self.collectionArray().length; i++) {
            var newItem = self.collectionArray()[i];
            newCollectionArray.push(self.prepItemForApp(newItem, this.structure, callbackArray));
        }
        self.collection = newCollectionArray;
    }
